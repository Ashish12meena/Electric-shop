...
public class CartDao implements CartService {
	...
	public String addProductToCart(String userId, String prodId, int prodQty) {
		...
		try {
			...
			if (rs.next()) {
				...
				ProductModel product = new ProductDao().getProductDetails(prodId);
				...
				if (availableQty < prodQty) {
					...
					DemandModel demandModel = new DemandModel(userId, product.getProdId(), prodQty - availableQty);

					DemandDao dao = new DemandDao();

					boolean flag = dao.addProduct(demandModel);
	...
	public List<CartModel> getAllCartItems(String userId) {
		List<CartModel> items = new ArrayList<CartModel>();
